...
5. Run app via `uvicorn src.main:app --host 127.0.0.1 --port 8090 --reload`

# Usage
1. Feed app with:
```
curl -X POST http://localhost:8090/ingestions/ \
-H "Content-Type: application/json" \
-d '{"file_path": "input_docs/documents_1.jsonl"}'
```
add more files via changing the body.\
2. Get all documents: `curl http://localhost:8090/documents`\
3. Specific document: `curl http://localhost:8090/documents/312` where 312 is an ID.\
